...
import { WORLDS, type World } from '@/features/story/lib/worlds'
import { startNewStory, continueStory } from '@/app/actions/storyActions'
import type { StoryMeta } from '@/app/lib/storyState'
import { WorldDetails } from '@/features/story/components/WorldDetails'
...
function CreateStoryContent() {
  ...
  const handleStartNewStory = async () => {
    ...
    try {
      ...
      if (result.error) {
        ...
        router.push(`/apps/story/${result.storyId}`)
      }
    } catch (error) {
  ...
  const handleViewStory = (storyId: string) => {
    router.push(`/apps/story/${storyId}`)
  }
      ...
      <header className="story-header">
        <div className="header-content">
          <div className="header-main">
            <h1 className="chronicle-title">故事书</h1>
            <p className="quest-subtitle">故事库</p>
          </div>
                  ...
                  return (
                    ...
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                        <div style={{ flex: 1 }}>
                          ...
                          <p style={{ fontSize: '14px', color: '#888', marginBottom: '8px' }}>
                            {world.displayName} · 第 {story.lastDay} 章 · {formatDate(story.createdAt)}
                          </p>
                          {story.lastSummary && (
                      ...
                      <div style={{ display: 'flex', gap: '12px', marginTop: '16px' }}>
                        ...
                        >
                          {isContinuing === story.storyId ? '加载中...' : '继续'}
                        </button>
                      </div>
                    </div>
                  )
