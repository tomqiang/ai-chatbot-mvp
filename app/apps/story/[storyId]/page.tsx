...
import StoryComposer from '@/features/story/components/StoryComposer'
import StoryEntry from '@/features/story/components/StoryEntry'
import StorySnapshot from '@/features/story/components/StorySnapshot'
import StoryEmptyState from '@/features/story/components/StoryEmptyState'
import RewriteModal from '@/features/story/components/RewriteModal'
import SettingsDrawer from '@/features/story/components/SettingsDrawer'
import { getWorldById, DEFAULT_WORLD_UI, type WorldUI } from '@/features/story/lib/worlds'
...
export default function StoryPage() {
  ...
  const [worldUI, setWorldUI] = useState<WorldUI>(DEFAULT_WORLD_UI)
  ...
  const checkStoryExists = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Set world UI config based on worldId
        if (meta.worldId) {
        ...
        // Redirect to story library with error after a brief delay
        setTimeout(() => {
          router.push('/apps/story?error=Story not found')
        }, 2000)
      }
    } catch (error) {
      setStoryExists(false)
      router.push('/apps/story?error=Failed to load story')
    }
  }
  ...
  return (
    <div className="chronicle-app">
      <StoryHeader 
        ...
        onBackToLibrary={() => router.push('/apps/story')}
        worldUI={worldUI}
      />
      ...
      <main className="chronicle-main">
        <div className="chronicle-content">
          ...
          <StorySnapshot
            ...
            questLabel={worldUI.questLabel}
            questText={worldUI.questText}
          />
          ...
          <StoryComposer 
            ...
            placeholder={worldUI.todaysEventPlaceholder}
          />
          ...
          {/* Empty state */}
          {entries.length === 0 && !isLoading && <StoryEmptyState />}
