...
export async function POST(
  ...
  try {
    ...
    // Load latest entry
    const latestEntry = await getStoryEntryByDay(storyId, lastDay)
    if (!latestEntry) {
      return NextResponse.json(
    ...
    console.log(`Rewriting ${storyId}, Day ${lastDay} with new event: ${userEvent}`)

    // Generate request ID for logging
    const requestId = `${Date.now()}-${Math.random().toString(36).substring(2, 9)}`

    // Generate summary up to previous day (for authoritative context)
    const allEntries = await loadStoryEntries(storyId)
    ...
    const previousDayEntry = lastDay > 1 
      ? await getStoryEntryByDay(storyId, lastDay - 1)
      : null
    ...
    const bundle = await generateChapterBundle(
      ...
      { day: lastDay, revision: latestEntry.revision || 1, requestId, storyId, worldId: state.worldId }
    )
    ...
    await updateStoryEntry(storyId, lastDay, {
      userEvent,
